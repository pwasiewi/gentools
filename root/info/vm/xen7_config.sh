...
#scp  rpmbuild/RPMS/x86_64/kernel-* xen04:
#scp  rpmbuild/RPMS/x86_64/kernel-* xen06:
...
echo "192.168.10.5${host01}/24 xenbr1" > /etc/ctdb/public_addresses
echo "192.168.10.5${host02}/24 xenbr1" >> /etc/ctdb/public_addresses
echo "192.168.10.5${host03}/24 xenbr1" >> /etc/ctdb/public_addresses
chkconfig ctdb on
#service ctdb restart

#init_gluster4 "xen" "03" "04" "06" "vol0" 3
#init_gluster4 "xen" "03" "04" "06" "vol1" 3
#service ctdb restart
#ctdb status
...
#echo "10.10.10.1${host04}" >> /etc/ansible/hosts
ssh-keygen -t rsa -b 2048 -N '' -f ~/.ssh/id_rsa
#ssh-copy-id -i root@xen03
#ssh-copy-id -i root@xen04
#ssh-copy-id -i root@xen05
#ssh-copy-id -i root@xen06
...
#for node in xen03 xen04 xen06 osd3 osd4 osd6; do ssh-copy-id $node ; done
...
cd ceph-deploy/
ceph-deploy purge xen03 xen04 xen06
ceph-deploy purgedata xen03 xen04 xen06
ceph-deploy forgetkeys
ceph-deploy new xen03 xen04 xen06
#ceph-deploy install --release jewel --no-adjust-repos xen03 xen04 xen06
#ceph-deploy install --release jewel xen03 xen04 xen06
ceph-deploy install --repo-url http://download.ceph.com/rpm-jewel/el7/ xen03 xen04 xen06
ceph-deploy --overwrite-conf mon create-initial

ceph-deploy gatherkeys xen03
...
ceph-deploy admin xen03 xen04 xen06 osd3 osd4 osd6
...
ceph-deploy rgw create xen03 xen04 xen06
#cephfs
ceph-deploy mds create xen03 xen04 xen06
ceph -s #ceph status
ceph osd tree
ceph mon_status
...
mkdir /mnt/mycephfs
mount -t ceph xen03:6789:/ /mnt/mycephfs -o name=admin,secret=`cat ./ceph.client.admin.keyring | grep key | cut -f 2 | sed 's/key = //g'`
__EOF__


echo 'Configure NIC1 (192.168.10.15?) i NIC2 (10.10.10.1?)'
echo 'for node in xen03 xen04 xen06 osd3 osd4 osd6; do ssh-copy-id $node ; done'

echo 'FOR GLUSTER Execute on one server:'
echo 'gluster peer probe xen03'
echo 'gluster peer probe xen04'
echo 'gluster peer probe xen06'
#echo 'gluster peer probe xen08'
echo 'init_gluster4 "xen" "03" "04" "06" "vol0" 3'
echo 'init_gluster4 "xen" "03" "04" "06" "vol1" 3'
